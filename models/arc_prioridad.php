<?php
	class svrarc_prioridad extends Conectar{
		...
		public function mdl_obtener_vista_00(
			...
			$ssql_sentencia = "call apr_web_arc_prioridad_obtener_vista_00 ()";
		...
		public function mdl_agregar(
			$psprioridad_codigo,
			$psprioridad_descripcion,
			$psprioridad_abreviatura,
			$psauditoria_usuario
		){
			$nhandle_conexion_bd = parent::_crear_conexion_bd();
			parent::set_names();

			$ssql_sentencia = "call apr_web_arc_prioridad_agregar (?,?,?,?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $psprioridad_codigo);
			$ssql_objeto->bindValue(2, $psprioridad_descripcion);
			$ssql_objeto->bindValue(3, $psprioridad_abreviatura);
		...
		public function mdl_eliminar(
			...
			$ssql_sentencia ="call apr_web_arc_prioridad_eliminar (?,?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
		...
		public function mdl_buscar(
			...
			$ssql_sentencia = "call apr_web_arc_prioridad_buscar (?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $pscodigo_aleatorio);
			$ssql_objeto->execute();
			return $ssql_resultado = $ssql_objeto->fetchAll(PDO::FETCH_ASSOC);
		...
		public function mdl_buscar_dependencia(
			...
			$nhandle_conexion_bd = parent::_crear_conexion_bd();
			parent::set_names();
			$ssql_sentencia = "call apr_web_arc_prioridad_buscar_dependencia (?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $pscodigo_aleatorio);
			$ssql_objeto->execute();
			return $ssql_resultado = $ssql_objeto->fetchAll(PDO::FETCH_ASSOC);
