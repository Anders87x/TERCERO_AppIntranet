<?php
	class svrarc_tipo_documento extends Conectar{
		...
		public function mdl_obtener_vista_00(
			...
			$ssql_sentencia = "call apr_web_arc_tipo_documento_obtener_vista_00 ()";
		...
		public function mdl_agregar(
			...
			$pstipo_documento_descripcion,
			$pstipo_documento_breve,
			$pstipo_documento_sunat,
			$psauditoria_usuario
		){
			$nhandle_conexion_bd = parent::_crear_conexion_bd();
			parent::set_names();

			$ssql_sentencia = "call apr_web_arc_tipo_documento_agregar (?,?,?,?,?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $pstipo_documento_codigo);
			$ssql_objeto->bindValue(2, $pstipo_documento_descripcion);
			$ssql_objeto->bindValue(3, $pstipo_documento_breve);
			$ssql_objeto->bindValue(4, $pstipo_documento_sunat);
		...
		public function mdl_eliminar(
			...
			$ssql_sentencia ="call apr_web_arc_tipo_documento_eliminar (?,?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
		...
		public function mdl_buscar(
			...
			$ssql_sentencia = "call apr_web_arc_tipo_documento_buscar (?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $pscodigo_aleatorio);
			$ssql_objeto->execute();
			return $ssql_resultado = $ssql_objeto->fetchAll(PDO::FETCH_ASSOC);
		...
		public function mdl_buscar_dependencia(
			...
			$nhandle_conexion_bd = parent::_crear_conexion_bd();
			parent::set_names();
			$ssql_sentencia = "call apr_web_arc_tipo_documento_buscar_dependencia (?)";
			$ssql_objeto = $nhandle_conexion_bd->prepare($ssql_sentencia);
			$ssql_objeto->bindValue(1, $pscodigo_aleatorio);
			$ssql_objeto->execute();
			return $ssql_resultado = $ssql_objeto->fetchAll(PDO::FETCH_ASSOC);
